...
use boson_dom::{Document, DocumentConfig, FontContext, ScrollBehavior, ScrollLogicalPosition};
...
fn pointer_event(x: f32, y: f32) -> BosonPointerEvent {
    BosonPointerEvent {
        id: BosonPointerId::Mouse,
        is_primary: true,
...
fn wheel_at(doc: &mut HtmlDocument, x: f32, y: f32, delta_x: f64, delta_y: f64) {
    ...
    doc.handle_ui_event(UiEvent::Wheel(BosonWheelEvent {
...
fn scroll_into_view_aligns_each_axis() {
    ...
    for (position, expected_x, expected_y) in [
        ...
        doc.set_viewport_scroll(boson_dom::Point::ZERO);
...
fn scroll_into_view_nearest_preserves_visible_target() {
    let mut doc = layout_doc(ALIGNMENT_HTML);
    let target = doc.query_selector("#alignment-target").unwrap().unwrap();
    doc.set_viewport_scroll(boson_dom::Point { x: 500.0, y: 900.0 });
