...
use boson_test_harness::{Harness, HarnessOptions, pointer_event};
use boson_traits::events::{
    BosonPointerEvent, BosonPointerId, MouseEventButton, MouseEventButtons, UiEvent,
};
use keyboard_types::Modifiers;

fn event(id: BosonPointerId, x: f32, y: f32) -> BosonPointerEvent {
    pointer_event(
        id,
        x,
        y,
...
fn finger_input_generates_touch_events() {
    ...
    let finger = BosonPointerId::Finger(0);
...
fn mouse_cancel_generates_pointercancel_without_touch_or_mouse() {
    let mut harness = target_harness();
    let mouse = BosonPointerId::Mouse;
    let names = harness.dispatch_recorded([
