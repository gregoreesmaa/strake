...
//! "invalid key" slab panic in boson-dom's `DocumentMutator` later in the
//! same render (matching the stack trace reported in the PR).

use boson_test_harness::HarnessOptions;
use dioxus::prelude::*;
...
struct Harness {
    inner: boson_test_harness::Harness<DioxusDocument>,
    generation: Rc<Cell<usize>>,
}
...
impl Harness {
    fn new(view: fn(usize) -> Element) -> Self {
        ...
        let inner = boson_test_harness::Harness::from_vdom(vdom, HarnessOptions::default());
...
/// "invalid key" inside boson-dom's `DocumentMutator` (in `node_at_path`
...
/// in dioxus-core and NodeId (slab slot) reuse in boson-dom.
