...
struct Harness {
    inner: strake_test_harness::Harness<DioxusDocument>,
    generation: Rc<Cell<usize>>,
}
...
impl Harness {
    fn new(view: fn(usize) -> Element) -> Self {
        ...
        let inner = strake_test_harness::Harness::from_vdom(vdom, HarnessOptions::default());
...
/// "invalid key" inside strake-dom's `DocumentMutator` (in `node_at_path`
...
/// in dioxus-core and NodeId (slab slot) reuse in strake-dom.
