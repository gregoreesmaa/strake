...
use boson_dom::{BaseDocument, ElementData, Node, NodeId, local_name};
use boson_traits::devtools::DevtoolSettings;
...
pub struct BosonDomPainter<'dom, 'a> {
    ...
    pub(crate) hovered_scrollbar: Option<boson_dom::node::ScrollbarRef>,
    #[cfg(feature = "scrollbars")]
    pub(crate) scrollbar_drag_target: Option<boson_dom::node::ScrollbarRef>,
...
impl<'dom, 'a> BosonDomPainter<'dom, 'a> {
    /// Create a new BosonDomPainter for the given document
    pub fn new(
    ...
    ///
    /// In Boson, text styling gets its attributes from its container element/resolved styles
...
struct ElementCx<'dom, 'a> {
    context: &'dom BosonDomPainter<'dom, 'a>,
    frame: CssBox,
...
impl ElementCx<'_, '_> {
    ...
    fn draw_scrollbars(&self, scene: &mut impl PaintScene) {
        ...
        // content.
        let dark_scheme =
            self.context.dom.viewport().color_scheme == boson_traits::shell::ColorScheme::Dark;
    ...
    fn draw_sub_document(&self, scene: &mut impl PaintScene) {
        if let Some(sub_doc) = self.element.sub_doc_data().map(|doc| doc.inner()) {
            ...
            let painter = BosonDomPainter::new(
                &sub_doc,
                scale,
                width,
                height,
...
impl<'dom, 'a> std::ops::Deref for ElementCx<'dom, 'a> {
    type Target = BosonDomPainter<'dom, 'a>;
