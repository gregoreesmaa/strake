//! Paint a [`boson_dom::BaseDocument`] by pushing [`anyrender`] drawing commands into
...
use boson_dom::{BaseDocument, NodeId, util::Color};
use render::BosonDomPainter;
...
/// Paint a [`boson_dom::BaseDocument`] by pushing drawing commands into
/// an impl [`anyrender::PaintScene`].
///
...
pub fn paint_scene(
    ...
    let generator = BosonDomPainter::new(
        doc,
        scale,
        width,
        height,
...
fn process_custom_widget_node(
    ...
    use boson_dom::node::{CustomWidgetStatus, ProxyRenderContext};
