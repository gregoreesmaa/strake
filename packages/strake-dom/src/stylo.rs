...
///
/// Since StrakeNodes are not persistent (IE we don't keep the pointers around between frames), we choose to just implement
/// the tree structure in the nodes themselves, and temporarily give out pointers during the layout phase.
type StrakeNode<'a> = &'a Node;

impl<'a> TDocument for StrakeNode<'a> {
    type ConcreteNode = StrakeNode<'a>;
...
impl NodeInfo for StrakeNode<'_> {
...
impl<'a> TShadowRoot for StrakeNode<'a> {
    type ConcreteNode = StrakeNode<'a>;
...
impl<'a> TNode for StrakeNode<'a> {
    type ConcreteElement = StrakeNode<'a>;
    type ConcreteDocument = StrakeNode<'a>;
    type ConcreteShadowRoot = StrakeNode<'a>;
...
impl selectors::Element for StrakeNode<'_> {
...
impl<'a> TElement for StrakeNode<'a> {
    type ConcreteNode = StrakeNode<'a>;
...
impl std::hash::Hash for StrakeNode<'_> {
...
#[allow(unsafe_code)]
impl<'dom> DomTraversal<StrakeNode<'dom>> for RecalcStyle<'_> {
    fn process_preorder<F: FnMut(StrakeNode<'dom>)>(
        &self,
        traversal_data: &PerLevelTraversalData,
        context: &mut StyleContext<StrakeNode<'dom>>,
        node: StrakeNode<'dom>,
        note_child: F,
    ) {
    ...
    fn process_postorder(
        &self,
        _style_context: &mut StyleContext<StrakeNode<'dom>>,
        _node: StrakeNode<'dom>,
    ) {
...
fn assert_size_of_equals() {
    ...
    // let size = mem::size_of::<StyleSharingCandidate<StrakeNode>>();
    // dbg!(size);
}
