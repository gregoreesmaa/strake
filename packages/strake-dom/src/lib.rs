//! The core DOM abstraction in Strake
//!
...
//! to use a wrapper:
//!
//!  - [`HtmlDocument`](https://docs.rs/strake-html/latest/strake_html/struct.HtmlDocument.html) from the [strake-html](https://docs.rs/strake-html) crate.
//!    Allows you to parse HTML (or XHTML) into a Strake [`BaseDocument`], and can be combined with a markdown-to-html converter like [comrak](https://docs.rs/comrak)
...
//! separate crates, including html parsing ([strake-html](https://docs.rs/strake-html)), networking ([strake-net](https://docs.rs/strake-html)),
//! rendering ([strake-paint](https://docs.rs/strake-paint)) and windowing ([strake-shell](https://docs.rs/strake-shell)).
//!
//! Most of the functionality in this crates is provided through the  struct.
//!
//! `strake-dom` has a native Rust API that is designed for higher-level abstractions to be built on top (although it can also be used directly).
//!
...
pub use strake_traits::node_id::NodeId;
// Re-export taffy: it is part of strake-dom's public API (e.g. `Node::style`,
// `Node::final_layout`)
...
/// Convert a Strake [`NodeId`] into a [`taffy::NodeId`] (which wraps a `u64`).
#[inline]
pub fn taffy_node_id(id: NodeId) -> taffy::NodeId {
    taffy::NodeId::from(id.as_u64())
}

/// Convert a [`taffy::NodeId`] produced by [`taffy_node_id`] back into a Strake [`NodeId`].
#[inline]
