...
/// Parley Brush type for Strake which contains the Strake node id
...
impl TextInputData {
    ...
    pub(crate) fn apply_ime_event(
        ...
        font_ctx: &mut FontContext,
        layout_ctx: &mut LayoutContext<TextBrush>,
        event: StrakeImeEvent,
    ) -> Option<GeneratedTextInputEvent> {
        let editor = &mut self.editor;
        let mut driver = editor.driver(font_ctx, layout_ctx);

        match event {
            StrakeImeEvent::Enabled => {
                // Do nothing
                None
            }
            ...
            StrakeImeEvent::Commit(text) => {
                driver.insert_or_replace_selection(&text);
                Some(GeneratedTextInputEvent::Input)
            }
            StrakeImeEvent::Preedit(text, cursor) => {
                if text.is_empty() {
                    driver.clear_compose();
                } else {
