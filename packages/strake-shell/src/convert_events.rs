...
pub(crate) fn winit_ime_to_strake(event: Ime) -> StrakeImeEvent {
    match event {
        Ime::Enabled => StrakeImeEvent::Enabled,
        Ime::Disabled => StrakeImeEvent::Disabled,
        Ime::Preedit(text, cursor) => StrakeImeEvent::Preedit(text, cursor),
        Ime::Commit(text) => StrakeImeEvent::Commit(text),
        ...
        _ => {
            ...
            StrakeImeEvent::Disabled
        }
    }
}

pub(crate) fn winit_key_event_to_strake(
    event: &WinitKeyEvent,
    mods: WinitModifiers,
) -> StrakeKeyEvent {
    StrakeKeyEvent {
...
pub(crate) fn pointer_source_to_strake(source: &PointerSource) -> StrakePointerId {
    match source {
        PointerSource::Mouse => StrakePointerId::Mouse,
        PointerSource::Touch { finger_id, .. } => {
            StrakePointerId::Finger(finger_id.into_raw() as u64)
        }

        // TODO: TabletTool and Unknown events
        PointerSource::TabletTool { .. } => StrakePointerId::Pen,
        PointerSource::Unknown => StrakePointerId::Mouse,
        _ => {
            #[cfg(feature = "tracing")]
...
pub(crate) fn pointer_kind_to_strake(kind: &PointerKind) -> StrakePointerId {
    match kind {
        PointerKind::Mouse => StrakePointerId::Mouse,
        PointerKind::Touch(finger_id) => StrakePointerId::Finger(finger_id.into_raw() as u64),

        // TODO: TabletTool and Unknown events
        PointerKind::TabletTool(_) => StrakePointerId::Pen,
        PointerKind::Unknown => StrakePointerId::Mouse,
        _ => {
            #[cfg(feature = "tracing")]
...
pub(crate) fn button_source_to_strake(source: &ButtonSource) -> StrakePointerId {
    match source {
        ButtonSource::Mouse(_) => StrakePointerId::Mouse,
        ButtonSource::TabletTool { .. } => StrakePointerId::Pen,
        ButtonSource::Touch { finger_id, .. } => {
            StrakePointerId::Finger(finger_id.into_raw() as u64)
        }

        // TODO: Unknown events
        ButtonSource::Unknown(_) => StrakePointerId::Mouse,
        _ => {
            #[cfg(feature = "tracing")]
...
pub(crate) fn pointer_source_to_strake_details(source: &PointerSource) -> PointerDetails {
    match source {
