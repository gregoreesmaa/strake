use crate::event::{StrakeShellEvent, StrakeShellProxy};
...
pub struct StrakeApplication<Rend: WindowRenderer> {
    pub windows: HashMap<WindowId, View<Rend>>,
    pub pending_windows: Vec<WindowConfig<Rend>>,
    pub proxy: StrakeShellProxy,
    pub event_queue: Receiver<StrakeShellEvent>,
}

impl<Rend: WindowRenderer> StrakeApplication<Rend> {
    pub fn new(proxy: StrakeShellProxy, event_queue: Receiver<StrakeShellEvent>) -> Self {
        StrakeApplication {
            windows: HashMap::new(),
    ...
    pub fn handle_strake_shell_event(
        &mut self,
        event_loop: &dyn ActiveEventLoop,
        event: StrakeShellEvent,
    ) {
        match event {
            StrakeShellEvent::Poll { window_id } => {
                if let Some(window) = self.windows.get_mut(&window_id) {
                    window.poll();
                };
            }
            StrakeShellEvent::CloseWindow { window_id } => {
            ...
            StrakeShellEvent::ResumeReady { window_id } => {
            ...
            StrakeShellEvent::RequestRedraw { doc_id } => {
            ...
            StrakeShellEvent::Accessibility { window_id, data } => {
                if let Some(window) = self.windows.get_mut(&window_id) {
                    match &*data {
            ...
            StrakeShellEvent::Embedder(_) => {
                // Do nothing. Should be handled by embedders (if required).
            }
            StrakeShellEvent::Navigate(_opts) => {
                // Do nothing. Should be handled by embedders (if required).
            }
            ...
            StrakeShellEvent::ResizeSettleCheck { window_id } => {
                if let Some(window) = self.windows.get_mut(&window_id) {
...
impl<Rend: WindowRenderer> ApplicationHandler for StrakeApplication<Rend> {
    fn can_create_surfaces(&mut self, event_loop: &dyn ActiveEventLoop) {
        ...
        // dispatch StrakeShellEvent::ResumeReady when init completes. Either way
    ...
    fn window_event(
        ...
        self.proxy.send_event(StrakeShellEvent::Poll { window_id });
    }

    fn proxy_wake_up(&mut self, event_loop: &dyn ActiveEventLoop) {
        while let Ok(event) = self.event_queue.try_recv() {
            self.handle_strake_shell_event(event_loop, event);
        }
    }
...
#[cfg(target_os = "macos")]
impl<Rend: WindowRenderer> ApplicationHandlerExtMacOS for StrakeApplication<Rend> {
