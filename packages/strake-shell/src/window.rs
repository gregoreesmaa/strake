...
pub struct View<Rend: WindowRenderer> {
    ...
    ///   [`StrakePointerEvent`] so that touch events can report all concurrent
    ...
    pub active_events: Arc<AtomicRefCell<Vec<StrakePointerEvent>>>,
...
impl<Rend: WindowRenderer> View<Rend> {
    pub fn init(
        ...
        let shell_provider = StrakeShellProvider::new(winit_window.clone(), proxy.clone());
...
impl<Rend: WindowRenderer> View<Rend> {
    /// Start resuming the renderer. Dispatches [`StrakeShellEvent::ResumeReady`]
    ...
    pub fn resume(&mut self) {
        ...
        self.renderer
            .resume(Arc::new(self.window.clone()), width, height, move || {
                proxy.send_event(StrakeShellEvent::ResumeReady { window_id });
            });
    }

    /// Finalize a previously-started resume. Should be called in response to a
    /// [`StrakeShellEvent::ResumeReady`] event. Paints the first frame and
    ...
    /// reference cycle.
    fn set_active_pointer(&self, event: &StrakePointerEvent) {
    ...
    fn update_active_pointer(&self, event: &StrakePointerEvent) {
        let mut active = self.active_events.borrow_mut();
    ...
    fn remove_active_pointer(&self, id: StrakePointerId) -> bool {
        let mut active = self.active_events.borrow_mut();
    ...
    fn schedule_resize_settle_check(&mut self, delay_ms: u32) {
        ...
        let cb = Closure::once_into_js(move || {
            proxy.send_event(StrakeShellEvent::ResizeSettleCheck { window_id });
        });
    ...
    pub fn handle_winit_event(&mut self, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::Ime(ime_event) => {
                self.doc.handle_ui_event(UiEvent::Ime(winit_ime_to_strake(ime_event)));
                self.request_redraw();
            },
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                ...
                let key_event_data = winit_key_event_to_strake(&event, self.keyboard_modifiers.state());
            ...
            WindowEvent::PointerLeft { position, primary, kind, .. } => {
                let id = pointer_kind_to_strake(&kind);
                ...
                if id != StrakePointerId::Mouse && self.remove_active_pointer(id) {
            ...
            WindowEvent::PointerMoved { position, source, primary, .. } => {
                self.pointer_pos = position;
                let id = pointer_source_to_strake(&source);
                let event = StrakePointerEvent {
                    id,
                    is_primary: primary,
                    coords: self.pointer_coords(position),
                    ...
                    buttons: self.buttons,
                    mods: winit_modifiers_to_kbt_modifiers(self.keyboard_modifiers.state()),
                    details: pointer_source_to_strake_details(&source),
                    element: Default::default(),
                    active_pointers: Arc::clone(&self.active_events),
                };
                // Keep multi-touch positions current (no-op for non-active pointers).
                if id != StrakePointerId::Mouse {
            ...
            WindowEvent::PointerButton { button, state, primary, position, .. } => {
                let id = button_source_to_strake(&button);
                ...
                let pointer_event = StrakePointerEvent {
                    id,
                    is_primary: primary,
                    coords,
                    button,
                ...
                if id != StrakePointerId::Mouse && state == ElementState::Pressed {
                ...
                if id != StrakePointerId::Mouse && state == ElementState::Released {
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                let strake_delta = match delta {
                    winit::event::MouseScrollDelta::LineDelta(x, y) => StrakeWheelDelta::Lines(x as f64, y as f64),
                    winit::event::MouseScrollDelta::PixelDelta(pos) => StrakeWheelDelta::Pixels(pos.x, pos.y),
                    _ =>  {
                        #[cfg(feature = "tracing")]
                        tracing::error!("unsupported MouseScrollDelta variant: {delta:?}");
                        StrakeWheelDelta::Pixels(0., 0.)
                    }
                };

                let event = StrakeWheelEvent {
                    delta: strake_delta,
