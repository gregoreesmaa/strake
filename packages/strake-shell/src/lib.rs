...
pub use crate::application::StrakeApplication;
pub use crate::event::{StrakeShellEvent, StrakeShellProxy};
...
pub struct StrakeShellProvider {
    window: Arc<dyn Window>,
    proxy: StrakeShellProxy,
}
impl StrakeShellProvider {
    pub fn new(window: Arc<dyn Window>, proxy: StrakeShellProxy) -> Self {
        Self { window, proxy }
    }
}

impl ShellProvider for StrakeShellProvider {
    ...
    fn request_window_close(&self) {
        self.proxy.send_event(StrakeShellEvent::CloseWindow {
    ...
    fn get_clipboard_text(&self) -> Result<String, strake_traits::shell::ClipboardError> {
        let mut cb = arboard::Clipboard::new().unwrap();
        cb.get_text()
            .map_err(|_| strake_traits::shell::ClipboardError)
    }
    ...
    fn set_clipboard_text(&self, text: String) -> Result<(), strake_traits::shell::ClipboardError> {
        let mut cb = arboard::Clipboard::new().unwrap();
