...
impl<D: Document> Harness<D> {
    ...
    /// Read access to the underlying [`strake_dom::BaseDocument`]
    pub fn base(&self) -> DocGuard<'_> {
        self.doc.inner()
    }

    /// Write access to the underlying [`strake_dom::BaseDocument`]
    ...
    /// to application code.
    ///
    /// The events are driven directly against the underlying [`strake_dom::BaseDocument`],
    ...
    pub fn dispatch_recorded(&mut self, events: impl IntoIterator<Item = UiEvent>) -> Vec<String> {
        ...
        impl EventHandler for RecordingHandler {
            fn handle_event(
                &mut self,
                _chain: &[strake_traits::node_id::NodeId],
                event: &mut DomEvent,
