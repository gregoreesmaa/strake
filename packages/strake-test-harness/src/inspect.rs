...
use strake_dom::{Document, Node, NodeData};
use strake_traits::events::HitResult;
...
fn write_node(out: &mut String, doc: &strake_dom::BaseDocument, node: &Node, depth: usize) {
    for _ in 0..depth {
        out.push_str("  ");
    }
    ...
    match &node.data {
        ...
        NodeData::Element(data) => {
            ...
            if let Some(id) = node.attr(strake_dom::local_name!("id")) {
                write!(out, " #{id}").unwrap();
            }
            if let Some(class) = node.attr(strake_dom::local_name!("class")) {
