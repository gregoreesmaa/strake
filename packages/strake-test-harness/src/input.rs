...
//!
//! Free functions construct raw Strake input events; methods on [`Harness`] synthesize
...
/// Construct a [`StrakePointerEvent`] at page coordinates `(x, y)`
...
/// Construct a main-button mouse [`StrakePointerEvent`] at page coordinates `(x, y)`
pub fn mouse_pointer_event(x: f32, y: f32) -> StrakePointerEvent {
    pointer_event(
...
/// Construct a finger [`StrakePointerEvent`] at page coordinates `(x, y)`
pub fn touch_pointer_event(finger: u64, x: f32, y: f32) -> StrakePointerEvent {
    pointer_event(
        StrakePointerId::Finger(finger),
        x,
        y,
        MouseEventButton::Main,
        MouseEventButtons::from(MouseEventButton::Main),
...
/// Construct a [`StrakeKeyEvent`] for `key` in the given `state`.
///
/// For [`Key::Character`] keys, `text` is populated with the character(s) on press.
pub fn key_event(key: Key, state: KeyState, modifiers: Modifiers) -> StrakeKeyEvent {
...
impl<D: Document> Harness<D> {
    ...
    pub fn wheel_at(&mut self, x: f32, y: f32, delta_x: f64, delta_y: f64) {
        ...
        let event = StrakeWheelEvent {
            delta: StrakeWheelDelta::Pixels(delta_x, delta_y),
            coords: coords(x, y),
    ...
    pub fn ime(&mut self, event: StrakeImeEvent) {
