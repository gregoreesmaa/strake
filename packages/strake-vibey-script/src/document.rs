...
use strake_traits::events::{DomEvent, UiEvent};
use url::Url;
...
/// `<script>` tags, exposing DOM APIs backed by `strake-dom` to the scripts.
///
...
/// event listeners before Strake's default actions run.
...
impl ScriptDocument {
    ...
    /// an HTML parser provider (e.g. `strake_html::HtmlProvider`) set in its
    /// [`DocumentConfig`].
    ...
    /// [`poll`](strake_traits::Document::poll), and therefore don't need wakeups.
    ...
    /// [`next_timer_deadline`](Self::next_timer_deadline) and calling
    /// [`poll`](strake_traits::Document::poll): instead of sleeping until the
    ...
    fn record_error(&mut self, message: String) {
        #[cfg(feature = "tracing")]
        tracing::error!("strake-vibey-script: {message}");
    ...
    /// [`Node::synthetic_click_event`](strake_dom::Node::synthetic_click_event))
    /// through the document's event driver. The event is exposed to JavaScript
    /// event listeners, and Strake's default actions run unless prevented.
                ...
                if element.name.local == strake_dom::local_name!("script") {
                    ...
                    if is_js {
                        scripts.push(PendingScript {
                            src: element
                                .attr(strake_dom::local_name!("src"))
                                .map(str::to_string),
    ...
    fn arm_timer_thread(&mut self) {
        ...
        let sender = self.timer_thread.get_or_insert_with(|| {
            ...
            std::thread::Builder::new()
                .name("strake-vibey-script-timers".to_string())
