...
pub(crate) fn element_child_ids(doc: &strake_dom::BaseDocument, node_id: NodeId) -> Vec<NodeId> {
...
fn set_autofocus(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    if value {
        // strake-dom's autofocus handling expects the value "true"
        ...
        doc.get_node(node_id)
            ...
            .map(|element| {
                element
                    ...
                    .unwrap_or_else(|| element.attr(strake_dom::local_name!("checked")).is_some())
            })
            .unwrap_or(false)
    };
...
fn set_checked(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    // strake-dom's checked handling parses the value as a boolean
    write_attr(
        &ctx,
        node_id,
        "checked",
...
fn get_content(this: &JsValue, _: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let is_template = ctx
        ...
        .get_node(node_id)
        .and_then(|node| node.element_data())
        .is_some_and(|element| element.name.local == strake_dom::local_name!("template"));
    if !is_template {
        return Ok(JsValue::undefined());
    }
...
fn layout_value(
    this: &JsValue,
    context: &mut Context,
    f: impl FnOnce(&strake_dom::Node) -> f32,
) -> JsResult<JsValue> {
    let ctx = dom_ctx(context)?;
    let node_id = this_node_id(this)?;
    let mut doc = ctx.doc.borrow_mut();
...
// to the padding edge of its `offsetParent` (strake-dom's `offset_top_left`
...
fn current_scroll_offset(
        ...
        .is_some_and(|root| root.id == node_id)
    {
        ...
        doc.get_node(node_id)
            ...
            .unwrap_or(strake_dom::Point { x: 0.0, y: 0.0 })
    }
}
...
fn node_has_boxes(doc: &strake_dom::BaseDocument, node_id: NodeId) -> bool {
