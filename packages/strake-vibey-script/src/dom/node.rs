...
pub(crate) fn is_fragment(doc: &strake_dom::BaseDocument, node_id: NodeId) -> bool {
...
/// referencing them remain valid
fn detach_all(mutr: &mut strake_dom::DocumentMutator<'_>, node_ids: &[NodeId]) {
...
fn clone_node(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    enum CloneSrc {
        Element(strake_dom::QualName, Vec<strake_dom::Attribute>),
        Text(String),
        Other,
    }
