...
fn add_pointer_fields(event: &JsObject, data: &StrakePointerEvent, context: &mut Context) {
    define_value(
        event,
        "clientX",
...
fn add_key_fields(event: &JsObject, data: &StrakeKeyEvent, context: &mut Context) {
...
/// Create a JS event object for a Strake [`DomEventData`], populating
    ...
    match data {
        ...
        DomEventData::Wheel(wheel) => {
            let (delta_x, delta_y, delta_mode) = match wheel.delta {
                StrakeWheelDelta::Lines(x, y) => (x, y, 1),
                StrakeWheelDelta::Pixels(x, y) => (x, y, 0),
            };
