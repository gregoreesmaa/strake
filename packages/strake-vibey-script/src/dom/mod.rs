//! JavaScript DOM API bindings backed by `strake-dom`.
//!
...
//! [`node_wrapper`]) whose native data is a [`NodeRef`] holding the `strake-dom`
        ...
        let is_body = matches!(
            ...
            Some(NodeData::Element(element))
                if element.name.local == strake_dom::local_name!("body")
                    || element.name.local == strake_dom::local_name!("frameset")
        );
        (proto, is_body)
    };
...
pub(crate) fn collect_matching_descendants(
    ...
    root_id: NodeId,
    filter: impl Fn(&strake_dom::ElementData) -> bool,
) -> Vec<NodeId> {
...
pub(crate) fn matches_class_names(element: &strake_dom::ElementData, class_names: &[&str]) -> bool {
    let class_attr = element.attr(strake_dom::local_name!("class")).unwrap_or("");
    class_names
        .iter()
...
pub(crate) fn wrap_style_object(obj: JsObject, context: &mut Context) -> JsValue {
    let wrapper = context
        .global_object()
        .get(boa_engine::js_string!("__strake_wrap_style"), context)
        .ok()
        .and_then(|value| value.as_object())
