...
(function () {
    ...
    globalThis.__strake_wrap_style = function (native) {
        return new Proxy(native, {
            ...
            has(target, prop) {
                ...
                return (
                    isCssPropName(prop) && __strake_css_property_supported(toKebab(prop))
                );
            },
        });
    };
    ...
    // strake-vibey-script elements share a single prototype, so tag-specific
...
impl ModuleLoader for StrakeModuleLoader {
...
pub(crate) struct ScriptRuntime {
    pub context: Context,
    pub ctx: DomCtx,
    module_loader: Rc<StrakeModuleLoader>,
}
...
impl ScriptRuntime {
    pub fn new(
        ...
        let module_loader = Rc::new(StrakeModuleLoader {
            fetcher,
        ...
        // - TimeoutExtension: strake-vibey-script has its own setTimeout/setInterval/
        ...
        let navigator = ObjectInitializer::new(&mut context)
            .property(
                js_string!("userAgent"),
                js_string!("Mozilla/5.0 (compatible; Strake)"),
                Attribute::all(),
            )
            .build();
        ...
        register_global_fn(&mut context, "__strake_send_message", 1, send_message);

        // CSS property support check, used by the style Proxy's `has` trap
        register_global_fn(
            &mut context,
            "__strake_css_property_supported",
            1,
        ...
        runtime.eval_internal(BOOTSTRAP_JS, "<strake-bootstrap>");
        ...
        let code: Option<String> = {
            ...
            while let Some(node_id) = stack.pop() {
                ...
                if let Some(element) = node.element_data() {
                    if element.name.local == strake_dom::local_name!("body") {
                        code = element
                            .attr(strake_dom::local_name!("onload"))
                            .map(str::to_string);
                        break;
                    }
                }
    ...
    pub fn sync_named_element_globals(&mut self) {
        ...
        let ids: Vec<(String, NodeId)> = {
            ...
            while let Some(node_id) = stack.pop() {
                ...
                if let Some(id) = node
                    .element_data()
                    .and_then(|element| element.attr(strake_dom::local_name!("id")))
                {
                    if !id.is_empty() {
    ...
    pub fn dispatch_dom_event(
        ...
        // inputs. Strake only generates `input` events, so synthesise the `change`
        // event here.
    ...
    fn target_is_checkbox_or_radio(&self, node_id: NodeId) -> bool {
        ...
        doc.get_node(node_id)
            ...
            .is_some_and(|element| {
                element.name.local == strake_dom::local_name!("input")
                    && matches!(
                        element.attr(strake_dom::local_name!("type")),
    ...
    fn dispatch_event_inner(
        ...
        // Feed `preventDefault` / `stopPropagation` back into Strake
...
/// `window.scrollTo`/`window.scroll`: scroll the viewport, which strake-dom
...
fn css_property_supported(
    ...
    Ok(JsValue::from(strake_dom::css_property_is_supported(&name)))
}
