...
use boson_dom::node::Attribute;
use boson_dom::{DocumentMutator, HtmlParserProvider, NodeId};
use html5ever::{
    QualName,
...
/// Convert an html5ever Attribute which uses tendril for its value to a boson Attribute
/// which uses String.
fn html5ever_to_boson_attr(attr: html5ever::Attribute) -> Attribute {
    Attribute {
        name: attr.name,
...
impl HtmlParserProvider for HtmlProvider {
    ...
    fn parse_document(
        &self,
        html: &str,
        config: boson_dom::DocumentConfig,
    ) -> Box<dyn boson_dom::Document> {
...
impl<'m, 'doc> TreeSink for DocumentHtmlParser<'m, 'doc> {
    ...
    fn create_element(
        ...
        let attrs = attrs.into_iter().map(html5ever_to_boson_attr).collect();
