...
use strake_dom::{BaseDocument, ElementData, Node, NodeId, local_name};
...
pub struct StrakeDomPainter<'dom, 'a> {
    ...
    pub(crate) hovered_scrollbar: Option<strake_dom::node::ScrollbarRef>,
    #[cfg(feature = "scrollbars")]
    pub(crate) scrollbar_drag_target: Option<strake_dom::node::ScrollbarRef>,
...
impl<'dom, 'a> StrakeDomPainter<'dom, 'a> {
    /// Create a new StrakeDomPainter for the given document
    pub fn new(
    ...
    ///
    /// In Strake, text styling gets its attributes from its container element/resolved styles
...
struct ElementCx<'dom, 'a> {
    context: &'dom StrakeDomPainter<'dom, 'a>,
    frame: CssBox,
...
impl ElementCx<'_, '_> {
    ...
    fn draw_scrollbars(&self, scene: &mut impl PaintScene) {
        ...
        // content.
        let dark_scheme =
            self.context.dom.viewport().color_scheme == strake_traits::shell::ColorScheme::Dark;
    ...
    fn draw_sub_document(&self, scene: &mut impl PaintScene) {
        if let Some(sub_doc) = self.element.sub_doc_data().map(|doc| doc.inner()) {
            ...
            let painter = StrakeDomPainter::new(
                &sub_doc,
                scale,
                width,
                height,
...
impl<'dom, 'a> std::ops::Deref for ElementCx<'dom, 'a> {
    type Target = StrakeDomPainter<'dom, 'a>;
