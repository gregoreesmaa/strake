use crate::BosonShellProvider;
use crate::convert_events::{
    button_source_to_boson, color_scheme_to_theme, pointer_kind_to_boson, pointer_source_to_boson,
    pointer_source_to_boson_details, theme_to_color_scheme, winit_ime_to_boson,
    winit_key_event_to_boson, winit_modifiers_to_kbt_modifiers,
};
use crate::event::{BosonShellEvent, BosonShellProxy, create_waker};
use anyrender::WindowRenderer;
use boson_dom::Document;
use boson_paint::paint_scene;
use boson_traits::events::{
    BosonPointerEvent, BosonPointerId, BosonWheelDelta, BosonWheelEvent, MouseEventButton,
    MouseEventButtons, PointerCoords, PointerDetails, UiEvent,
};
use boson_traits::shell::Viewport;
...
pub struct View<Rend: WindowRenderer> {
    ...
    ///   [`BosonPointerEvent`] so that touch events can report all concurrent
    ...
    pub active_events: Arc<AtomicRefCell<Vec<BosonPointerEvent>>>,
...
impl<Rend: WindowRenderer> View<Rend> {
    pub fn init(
        ...
        let shell_provider = BosonShellProvider::new(winit_window.clone(), proxy.clone());
...
impl<Rend: WindowRenderer> View<Rend> {
    /// Start resuming the renderer. Dispatches [`BosonShellEvent::ResumeReady`]
    ...
    pub fn resume(&mut self) {
        ...
        self.renderer
            .resume(Arc::new(self.window.clone()), width, height, move || {
                proxy.send_event(BosonShellEvent::ResumeReady { window_id });
            });
    }

    /// Finalize a previously-started resume. Should be called in response to a
    /// [`BosonShellEvent::ResumeReady`] event. Paints the first frame and
    ...
    /// reference cycle.
    fn set_active_pointer(&self, event: &BosonPointerEvent) {
    ...
    fn update_active_pointer(&self, event: &BosonPointerEvent) {
        let mut active = self.active_events.borrow_mut();
    ...
    fn remove_active_pointer(&self, id: BosonPointerId) -> bool {
        let mut active = self.active_events.borrow_mut();
    ...
    fn schedule_resize_settle_check(&mut self, delay_ms: u32) {
        ...
        let cb = Closure::once_into_js(move || {
            proxy.send_event(BosonShellEvent::ResizeSettleCheck { window_id });
        });
    ...
    pub fn handle_winit_event(&mut self, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::Ime(ime_event) => {
                self.doc.handle_ui_event(UiEvent::Ime(winit_ime_to_boson(ime_event)));
                self.request_redraw();
            },
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                ...
                let key_event_data = winit_key_event_to_boson(&event, self.keyboard_modifiers.state());
            ...
            WindowEvent::PointerLeft { position, primary, kind, .. } => {
                let id = pointer_kind_to_boson(&kind);
                ...
                if id != BosonPointerId::Mouse && self.remove_active_pointer(id) {
            ...
            WindowEvent::PointerMoved { position, source, primary, .. } => {
                self.pointer_pos = position;
                let id = pointer_source_to_boson(&source);
                let event = BosonPointerEvent {
                    id,
                    is_primary: primary,
                    coords: self.pointer_coords(position),
                    ...
                    buttons: self.buttons,
                    mods: winit_modifiers_to_kbt_modifiers(self.keyboard_modifiers.state()),
                    details: pointer_source_to_boson_details(&source),
                    element: Default::default(),
                    active_pointers: Arc::clone(&self.active_events),
                };
                // Keep multi-touch positions current (no-op for non-active pointers).
                if id != BosonPointerId::Mouse {
            ...
            WindowEvent::PointerButton { button, state, primary, position, .. } => {
                let id = button_source_to_boson(&button);
                ...
                let pointer_event = BosonPointerEvent {
                    id,
                    is_primary: primary,
                    coords,
                    button,
                ...
                if id != BosonPointerId::Mouse && state == ElementState::Pressed {
                ...
                if id != BosonPointerId::Mouse && state == ElementState::Released {
            ...
            WindowEvent::MouseWheel { delta, .. } => {
                let boson_delta = match delta {
                    winit::event::MouseScrollDelta::LineDelta(x, y) => BosonWheelDelta::Lines(x as f64, y as f64),
                    winit::event::MouseScrollDelta::PixelDelta(pos) => BosonWheelDelta::Pixels(pos.x, pos.y),
                    _ =>  {
                        #[cfg(feature = "tracing")]
                        tracing::error!("unsupported MouseScrollDelta variant: {delta:?}");
                        BosonWheelDelta::Pixels(0., 0.)
                    }
                };

                let event = BosonWheelEvent {
                    delta: boson_delta,
