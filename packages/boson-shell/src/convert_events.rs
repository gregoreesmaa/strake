use boson_traits::events::{
    BosonImeEvent, BosonKeyEvent, BosonPointerId, KeyState, PointerDetails,
};
...
pub(crate) fn winit_ime_to_boson(event: Ime) -> BosonImeEvent {
    match event {
        Ime::Enabled => BosonImeEvent::Enabled,
        Ime::Disabled => BosonImeEvent::Disabled,
        Ime::Preedit(text, cursor) => BosonImeEvent::Preedit(text, cursor),
        Ime::Commit(text) => BosonImeEvent::Commit(text),
...
pub(crate) fn winit_key_event_to_boson(
    event: &WinitKeyEvent,
    mods: WinitModifiers,
) -> BosonKeyEvent {
    BosonKeyEvent {
...
pub(crate) fn pointer_source_to_boson(source: &PointerSource) -> BosonPointerId {
    match source {
        PointerSource::Mouse => BosonPointerId::Mouse,
        PointerSource::Touch { finger_id, .. } => {
            BosonPointerId::Finger(finger_id.into_raw() as u64)
        }

        // TODO: TabletTool and Unknown events
        PointerSource::TabletTool { .. } => BosonPointerId::Pen,
        PointerSource::Unknown => BosonPointerId::Mouse,
        _ => {
            #[cfg(feature = "tracing")]
...
pub(crate) fn pointer_kind_to_boson(kind: &PointerKind) -> BosonPointerId {
    match kind {
        PointerKind::Mouse => BosonPointerId::Mouse,
        PointerKind::Touch(finger_id) => BosonPointerId::Finger(finger_id.into_raw() as u64),

        // TODO: TabletTool and Unknown events
        PointerKind::TabletTool(_) => BosonPointerId::Pen,
        PointerKind::Unknown => BosonPointerId::Mouse,
        _ => {
            #[cfg(feature = "tracing")]
...
pub(crate) fn button_source_to_boson(source: &ButtonSource) -> BosonPointerId {
    match source {
        ButtonSource::Mouse(_) => BosonPointerId::Mouse,
        ButtonSource::TabletTool { .. } => BosonPointerId::Pen,
        ButtonSource::Touch { finger_id, .. } => {
            BosonPointerId::Finger(finger_id.into_raw() as u64)
        }

        // TODO: Unknown events
        ButtonSource::Unknown(_) => BosonPointerId::Mouse,
        _ => {
            #[cfg(feature = "tracing")]
...
pub(crate) fn pointer_source_to_boson_details(source: &PointerSource) -> PointerDetails {
    match source {
