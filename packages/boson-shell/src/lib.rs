...
pub use crate::application::BosonApplication;
pub use crate::event::{BosonShellEvent, BosonShellProxy};
...
use boson_traits::shell::FileDialogFilter;
...
pub struct BosonShellProvider {
    window: Arc<dyn Window>,
    proxy: BosonShellProxy,
}
impl BosonShellProvider {
    pub fn new(window: Arc<dyn Window>, proxy: BosonShellProxy) -> Self {
        Self { window, proxy }
    }
}

impl ShellProvider for BosonShellProvider {
    ...
    fn request_window_close(&self) {
        self.proxy.send_event(BosonShellEvent::CloseWindow {
    ...
    fn get_clipboard_text(&self) -> Result<String, boson_traits::shell::ClipboardError> {
        let mut cb = arboard::Clipboard::new().unwrap();
        cb.get_text()
            .map_err(|_| boson_traits::shell::ClipboardError)
    }
    ...
    fn set_clipboard_text(&self, text: String) -> Result<(), boson_traits::shell::ClipboardError> {
        let mut cb = arboard::Clipboard::new().unwrap();
