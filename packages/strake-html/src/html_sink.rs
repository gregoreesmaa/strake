...
/// which uses String.
fn html5ever_to_strake_attr(attr: html5ever::Attribute) -> Attribute {
    Attribute {
        name: attr.name,
...
impl HtmlParserProvider for HtmlProvider {
    ...
    fn parse_document(
        &self,
        html: &str,
        config: strake_dom::DocumentConfig,
    ) -> Box<dyn strake_dom::Document> {
...
impl<'m, 'doc> TreeSink for DocumentHtmlParser<'m, 'doc> {
    ...
    fn create_element(
        ...
        let attrs = attrs.into_iter().map(html5ever_to_strake_attr).collect();
