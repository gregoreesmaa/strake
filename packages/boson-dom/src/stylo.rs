...
use boson_traits::node_id::NodeId;
use std::ptr::NonNull;
use std::sync::Mutex;
...
///
/// Since BosonNodes are not persistent (IE we don't keep the pointers around between frames), we choose to just implement
/// the tree structure in the nodes themselves, and temporarily give out pointers during the layout phase.
type BosonNode<'a> = &'a Node;

impl<'a> TDocument for BosonNode<'a> {
    type ConcreteNode = BosonNode<'a>;
...
impl<'a> TShadowRoot for BosonNode<'a> {
    type ConcreteNode = BosonNode<'a>;
...
impl<'a> TNode for BosonNode<'a> {
    type ConcreteElement = BosonNode<'a>;
    type ConcreteDocument = BosonNode<'a>;
    type ConcreteShadowRoot = BosonNode<'a>;
...
impl selectors::Element for BosonNode<'_> {
...
impl<'a> TElement for BosonNode<'a> {
    type ConcreteNode = BosonNode<'a>;
...
impl std::hash::Hash for BosonNode<'_> {
...
#[allow(unsafe_code)]
impl<'dom> DomTraversal<BosonNode<'dom>> for RecalcStyle<'_> {
    fn process_preorder<F: FnMut(BosonNode<'dom>)>(
        &self,
        traversal_data: &PerLevelTraversalData,
        context: &mut StyleContext<BosonNode<'dom>>,
        node: BosonNode<'dom>,
        note_child: F,
    ) {
    ...
    fn process_postorder(
        &self,
        _style_context: &mut StyleContext<BosonNode<'dom>>,
        _node: BosonNode<'dom>,
    ) {
...
fn assert_size_of_equals() {
    ...
    // let size = mem::size_of::<StyleSharingCandidate<BosonNode>>();
    // dbg!(size);
}
