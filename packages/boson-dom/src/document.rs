...
use boson_traits::events::{DomEvent, HitResult, UiEvent};
use boson_traits::navigation::{DummyNavigationProvider, NavigationProvider};
use boson_traits::net::{AbortSignal, DummyNetProvider, NetProvider, Request};
use boson_traits::node_id::NodeId;
use boson_traits::shell::{DummyShellProvider, ShellProvider, Viewport};
...
/// be driven by [`boson-shell`](https://docs.rs/boson-shell)
...
mod zoom_tests {
    use super::*;
    use boson_traits::shell::ColorScheme;
    use std::sync::atomic::{AtomicUsize, Ordering};
...
mod hover_state_tests {
    ...
    /// parser lives in boson-html, which would be a circular dev-dependency).
...
mod font_face_override_tests {
    ...
    /// downstream HTML parser lives in `boson-html` (would be a circular
    ...
    fn font_face_overrides_alias_family_name() {
        ...
        let response = ResourceLoadResponse {
            ...
            result: Ok(Resource::Font(
                boson_traits::net::Bytes::from_static(crate::BULLET_FONT),
                FontFaceOverrides {
