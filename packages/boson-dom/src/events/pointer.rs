...
use boson_traits::{
    events::{
        BosonInputEvent, BosonPointerEvent, BosonPointerId, BosonWheelDelta, BosonWheelEvent,
...
pub(crate) fn handle_pointermove<F: FnMut(DomEvent)>(
    ...
    if buttons != MouseEventButtons::None && doc.drag_mode == DragMode::None {
        ...
        if dx.abs() > 2.0 || dy.abs() > 2.0 {
            match event.id {
                BosonPointerId::Mouse | BosonPointerId::Pen => {
        ...
        while let Some(node_id) = maybe_node_id {
            ...
            match el.name.local {
                local_name!("input") if el.attr(local_name!("type")) == Some("checkbox") => {
                    ...
                    dispatch_event(DomEvent::new(
                        node_id,
                        DomEventData::Input(BosonInputEvent { value }),
                    ));
                    generate_focus_events(
                        doc,
                        &mut |doc| {
...
pub(crate) fn handle_wheel<F: FnMut(DomEvent)>(
    doc: &mut BaseDocument,
    _: NodeId,
    event: BosonWheelEvent,
    mut dispatch_event: F,
) {
    let (scroll_x, scroll_y) = match event.delta {
        BosonWheelDelta::Lines(x, y) => (x * 20.0, y * 20.0),
        BosonWheelDelta::Pixels(x, y) => (x, y),
    };
