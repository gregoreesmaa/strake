use boson_traits::{
    events::{BosonImeEvent, BosonKeyEvent},
    node_id::NodeId,
    shell::ShellProvider,
};
...
/// Parley Brush type for Boson which contains the Boson node id
...
impl TextInputData {
    ...
    pub(crate) fn apply_ime_event(
        ...
        match event {
            BosonImeEvent::Enabled => {
                // Do nothing
                None
            }
            BosonImeEvent::Disabled => {
                driver.clear_compose();
                Some(GeneratedTextInputEvent::PreEditChange)
            }
            BosonImeEvent::Commit(text) => {
                driver.insert_or_replace_selection(&text);
                Some(GeneratedTextInputEvent::Input)
            }
            BosonImeEvent::Preedit(text, cursor) => {
                if text.is_empty() {
                    driver.clear_compose();
                } else {
            ...
            BosonImeEvent::DeleteSurrounding {
                before_bytes,
                after_bytes,
            } => {
                let _ = before_bytes;
