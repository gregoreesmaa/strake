//! The core DOM abstraction in Boson
//!
...
//! to use a wrapper:
//!
//!  - [`HtmlDocument`](https://docs.rs/boson-html/latest/boson_html/struct.HtmlDocument.html) from the [boson-html](https://docs.rs/boson-html) crate.
//!    Allows you to parse HTML (or XHTML) into a Boson [`BaseDocument`], and can be combined with a markdown-to-html converter like [comrak](https://docs.rs/comrak)
...
//! separate crates, including html parsing ([boson-html](https://docs.rs/boson-html)), networking ([boson-net](https://docs.rs/boson-html)),
//! rendering ([boson-paint](https://docs.rs/boson-paint)) and windowing ([boson-shell](https://docs.rs/boson-shell)).
//!
//! Most of the functionality in this crates is provided through the  struct.
//!
//! `boson-dom` has a native Rust API that is designed for higher-level abstractions to be built on top (although it can also be used directly).
//!
...
pub use boson_traits::node_id::NodeId;
// Re-export taffy: it is part of boson-dom's public API (e.g. `Node::style`,
// `Node::final_layout`)
...
/// Convert a Boson [`NodeId`] into a [`taffy::NodeId`] (which wraps a `u64`).
#[inline]
pub fn taffy_node_id(id: NodeId) -> taffy::NodeId {
    taffy::NodeId::from(id.as_u64())
}

/// Convert a [`taffy::NodeId`] produced by [`taffy_node_id`] back into a Boson [`NodeId`].
#[inline]
