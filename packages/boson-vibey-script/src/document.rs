...
use boson_html::{DocumentHtmlParser, HtmlProvider};
use boson_traits::events::{DomEvent, UiEvent};
use url::Url;
...
/// `<script>` tags, exposing DOM APIs backed by `boson-dom` to the scripts.
///
...
/// event listeners before Boson's default actions run.
...
impl ScriptDocument {
    ...
    /// an HTML parser provider (e.g. `boson_html::HtmlProvider`) set in its
    /// [`DocumentConfig`].
    ...
    /// [`poll`](boson_traits::Document::poll), and therefore don't need wakeups.
    ...
    /// [`next_timer_deadline`](Self::next_timer_deadline) and calling
    /// [`poll`](boson_traits::Document::poll): instead of sleeping until the
    ...
    /// `__boson_send_message(message)` native function.
    ///
    ...
    fn record_error(&mut self, message: String) {
        #[cfg(feature = "tracing")]
        tracing::error!("boson-vibey-script: {message}");
    ...
    /// [`Node::synthetic_click_event`](boson_dom::Node::synthetic_click_event))
    /// through the document's event driver. The event is exposed to JavaScript
    /// event listeners, and Boson's default actions run unless prevented.
    ...
    fn arm_timer_thread(&mut self) {
        ...
        let sender = self.timer_thread.get_or_insert_with(|| {
            ...
            std::thread::Builder::new()
                .name("boson-vibey-script-timers".to_string())
