...
pub(crate) fn is_fragment(doc: &boson_dom::BaseDocument, node_id: NodeId) -> bool {
...
/// referencing them remain valid
fn detach_all(mutr: &mut boson_dom::DocumentMutator<'_>, node_ids: &[NodeId]) {
...
fn clone_node(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    enum CloneSrc {
        Element(boson_dom::QualName, Vec<boson_dom::Attribute>),
        Text(String),
        Other,
    }
