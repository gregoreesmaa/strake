//! JavaScript DOM API bindings backed by `boson-dom`.
//!
...
//! [`node_wrapper`]) whose native data is a [`NodeRef`] holding the `boson-dom`
...
use boson_dom::node::NodeData;
use boson_dom::{LocalName, Namespace, NodeId, QualName};
        ...
        let is_body = matches!(
            ...
            Some(NodeData::Element(element))
                if element.name.local == boson_dom::local_name!("body")
                    || element.name.local == boson_dom::local_name!("frameset")
        );
        (proto, is_body)
    };
...
pub(crate) fn collect_matching_descendants(
    ...
    root_id: NodeId,
    filter: impl Fn(&boson_dom::ElementData) -> bool,
) -> Vec<NodeId> {
...
pub(crate) fn matches_class_names(element: &boson_dom::ElementData, class_names: &[&str]) -> bool {
    let class_attr = element.attr(boson_dom::local_name!("class")).unwrap_or("");
    class_names
        .iter()
...
pub(crate) fn wrap_style_object(obj: JsObject, context: &mut Context) -> JsValue {
    let wrapper = context
        .global_object()
        .get(boa_engine::js_string!("__boson_wrap_style"), context)
        .ok()
        .and_then(|value| value.as_object())
