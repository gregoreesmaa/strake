...
use boson_traits::events::{BosonKeyEvent, BosonPointerEvent, BosonWheelDelta, DomEventData};
...
fn add_pointer_fields(event: &JsObject, data: &BosonPointerEvent, context: &mut Context) {
    define_value(
        event,
        "clientX",
...
fn add_key_fields(event: &JsObject, data: &BosonKeyEvent, context: &mut Context) {
...
/// Create a JS event object for a Boson [`DomEventData`], populating
    ...
    match data {
        ...
        DomEventData::Wheel(wheel) => {
            let (delta_x, delta_y, delta_mode) = match wheel.delta {
                BosonWheelDelta::Lines(x, y) => (x, y, 1),
                BosonWheelDelta::Pixels(x, y) => (x, y, 0),
            };
