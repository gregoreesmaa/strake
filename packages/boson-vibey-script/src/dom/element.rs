...
use boson_dom::{LocalName, NodeId, QualName, ScrollBehavior, ScrollLogicalPosition};
...
pub(crate) fn element_child_ids(doc: &boson_dom::BaseDocument, node_id: NodeId) -> Vec<NodeId> {
...
fn set_autofocus(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    if value {
        // boson-dom's autofocus handling expects the value "true"
        ...
        doc.get_node(node_id)
            ...
            .map(|element| {
                element
                    ...
                    .unwrap_or_else(|| element.attr(boson_dom::local_name!("checked")).is_some())
            })
            .unwrap_or(false)
    };
...
fn set_checked(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    // boson-dom's checked handling parses the value as a boolean
    write_attr(
        &ctx,
        node_id,
        "checked",
...
fn selection_utf16_range(doc: &boson_dom::BaseDocument, node_id: NodeId) -> Option<(usize, usize)> {
...
fn get_content(this: &JsValue, _: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let is_template = ctx
        ...
        .get_node(node_id)
        .and_then(|node| node.element_data())
        .is_some_and(|element| element.name.local == boson_dom::local_name!("template"));
    if !is_template {
        return Ok(JsValue::undefined());
    }
...
fn layout_value(
    this: &JsValue,
    context: &mut Context,
    f: impl FnOnce(&boson_dom::Node) -> f32,
) -> JsResult<JsValue> {
    let ctx = dom_ctx(context)?;
    let node_id = this_node_id(this)?;
    let mut doc = ctx.doc.borrow_mut();
...
// to the padding edge of its `offsetParent` (boson-dom's `offset_top_left`
...
fn current_scroll_offset(doc: &boson_dom::BaseDocument, node_id: NodeId) -> boson_dom::Point<f64> {
    if doc
        .try_root_element()
        .is_some_and(|root| root.id == node_id)
    {
        ...
        doc.get_node(node_id)
            ...
            .unwrap_or(boson_dom::Point { x: 0.0, y: 0.0 })
    }
}
...
fn node_has_boxes(doc: &boson_dom::BaseDocument, node_id: NodeId) -> bool {
