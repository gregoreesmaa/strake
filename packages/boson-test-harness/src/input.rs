...
//!
//! Free functions construct raw Boson input events; methods on [`Harness`] synthesize
...
use boson_traits::events::{
    BosonImeEvent, BosonKeyEvent, BosonPointerEvent, BosonPointerId, BosonWheelDelta,
    BosonWheelEvent, KeyState, MouseEventButton, MouseEventButtons, Point, PointerCoords,
...
/// Construct a [`BosonPointerEvent`] at page coordinates `(x, y)`
...
/// Construct a main-button mouse [`BosonPointerEvent`] at page coordinates `(x, y)`
pub fn mouse_pointer_event(x: f32, y: f32) -> BosonPointerEvent {
    pointer_event(
        BosonPointerId::Mouse,
        x,
        y,
...
/// Construct a finger [`BosonPointerEvent`] at page coordinates `(x, y)`
pub fn touch_pointer_event(finger: u64, x: f32, y: f32) -> BosonPointerEvent {
    pointer_event(
        BosonPointerId::Finger(finger),
        x,
        y,
        MouseEventButton::Main,
        MouseEventButtons::from(MouseEventButton::Main),
...
/// Construct a [`BosonKeyEvent`] for `key` in the given `state`.
///
/// For [`Key::Character`] keys, `text` is populated with the character(s) on press.
pub fn key_event(key: Key, state: KeyState, modifiers: Modifiers) -> BosonKeyEvent {
...
impl<D: Document> Harness<D> {
    ...
    pub fn wheel_at(&mut self, x: f32, y: f32, delta_x: f64, delta_y: f64) {
        ...
        let event = BosonWheelEvent {
            delta: BosonWheelDelta::Pixels(delta_x, delta_y),
            coords: coords(x, y),
    ...
    pub fn ime(&mut self, event: BosonImeEvent) {
