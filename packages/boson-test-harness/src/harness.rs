...
use boson_dom::{DocGuard, DocGuardMut, Document, DocumentConfig};
...
impl<D: Document> Harness<D> {
    ...
    /// Read access to the underlying [`boson_dom::BaseDocument`]
    pub fn base(&self) -> DocGuard<'_> {
        self.doc.inner()
    }

    /// Write access to the underlying [`boson_dom::BaseDocument`]
    ...
    /// to application code.
    ///
    /// The events are driven directly against the underlying [`boson_dom::BaseDocument`],
    ...
    pub fn dispatch_recorded(&mut self, events: impl IntoIterator<Item = UiEvent>) -> Vec<String> {
        use boson_dom::{EventDriver, EventHandler};
        ...
        impl EventHandler for RecordingHandler {
            fn handle_event(
                &mut self,
                _chain: &[boson_traits::node_id::NodeId],
                event: &mut DomEvent,
