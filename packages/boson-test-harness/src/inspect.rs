...
use boson_dom::{Document, Node, NodeData};
use boson_traits::events::HitResult;
...
fn write_node(out: &mut String, doc: &boson_dom::BaseDocument, node: &Node, depth: usize) {
    for _ in 0..depth {
        out.push_str("  ");
    }
    ...
    match &node.data {
        ...
        NodeData::Element(data) => {
            ...
            if let Some(id) = node.attr(boson_dom::local_name!("id")) {
                write!(out, " #{id}").unwrap();
            }
            if let Some(class) = node.attr(boson_dom::local_name!("class")) {
