//! Minimal WASM proof: drives `StrakeApplication` on `wasm32-unknown-unknown` and
...
<html>
<head><title>strake-shell on WASM</title></head>
<body>
  <main>
    <h1>strake-shell, running in your browser</h1>
    <p class="lede">
      Laid out by <strong>strake-dom</strong>, painted by <strong>strake-paint</strong>,
      rendered through <strong>anyrender</strong>'s vello hybrid backend &mdash;
      driven by the same <code>StrakeApplication</code> that runs natively.
    </p>
    <ul>
      ...
      <li>A <code>StrakeShellEvent::ResumeReady</code> finalizes the first frame</li>
    </ul>
  </main>
</body>
<style>
...
pub fn start() -> Result<(), JsValue> {
    ...
    let canvas = document.get_element_by_id("strake-target").unwrap();
    ...
    let (proxy, rx) = StrakeShellProxy::new(event_loop.create_proxy());
    ...
    let mut app = StrakeApplication::<VelloHybridWindowRenderer>::new(proxy, rx);
